...
namespace QuickstartLoyalty
{
    ...
    class Membership
    {
        ...
        public Membership(ExcellRowData data)
        {
            Data = data;
        }
        ...
        { // Enrolls member on base tier
            ...
            if (Data == null)
            {
                member.TierId = Convert.ToString(2);//baseTierId.Id_;
                member.ProgramId = "6Vzwl6cTKWfQiNw1oIrWzH";//programId.Id_;
                member.Person = new Person();
                member.Person.Surname = "Loyal";
                member.Person.Forename = "Tally";
                member.Person.DisplayName = "Tally";
                member.Person.EmailAddress = baseEmail;
                member.Person.ExternalId = Convert.ToString(1234567);
                member.ExpiryDate = Timestamp.FromDateTime(DateTime.UtcNow.AddYears(1));
                member.Points = 0;
            }
            ...
            else
            {
                ...
                member.TierId = Data.TierId;
                member.ProgramId = Data.ProgramId;
                member.Person = new Person();
                if(names.Length > 1)
                {
                    member.Person.Surname = names[1];
                }
                member.ProfileImage = Data.PhotoPath;
                member.Person.Forename = names[0];
                member.Person.DisplayName = Data.Name;
                member.Person.EmailAddress = baseEmail;
                member.Person.ExternalId = Data.Id;
                member.ExpiryDate = Timestamp.FromDateTime(Data.ExpirationDate);
                member.Points = 0;
            }
